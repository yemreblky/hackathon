...
def delete_rss(rss_id):
    ...
    conn.commit()
    conn.close()
    return jsonify({"message": "RSS silindi"}), 200
...
def trigger_refresh():
    try:
        ...
        # 1. Adım: Collector çalışır, yeni haberleri veritabanına ekler[cite: 9]
        print("[1/2] RSS kaynakları taranıyor...")
        fetch_and_save_news()
        
        # 2. Adım: AI Processor çalışır, yeni haberleri Ollama'ya yollayıp skorlar[cite: 8]
...
# (Diğer rotaların altına bu yeni rotayı ekleyin)
